...
# 리스트는 순서가 있다. 앞에서부터 인덱스 0으로서, 각 인덱스로서 값에 접근이 가능하다.
print(subway)
...
# print(cabinet[45])
#런타임 오류가 나고 프로그램이 종료된다. 없는 키 값을 입력하는 경우 오류가 발생한다. 일종의 강제 언래핑 런타임 오류가 나는 것이다.

print(cabinet.get(45))
#겟을 사용하면 none을 출력해주고 프로그램이 종료되지 않는다. 일종의 옵셔널 바인딩과 비슷하다.
#안전하게 데이터 조회가 가능하다는 것이다.
...
#이렇게 하면 키와 밸류값의 튜플로 딕셔너리 아이템을 리턴해준다.
...
# 리스트는 append 이지만 set에서는 add키워드를 통해서 집합 값 추가가 가능하다.
