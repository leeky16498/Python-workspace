...
try: 
    ...
    # 사전 지정된 에러타입에 따라서 처리 결과를 지정해줄 수 있다.
...
try:
    ...
    #발생하는 에러를 as 키워드로 변수에 담아서 표현해줄 수 있다.
...
# 클래스를 작성하고, Exception 클래스를 상속한다.
# 그러면 에러처리를 위한 클래스가 된다.
...
## 에러를 발생시킬때는  raise 키워드를 사용한다.
## 에러가 발생하는 함수를 수행할 때는 try 키워드를 사용한다.
